...
export default function Landing() {
  ...
  const scrollToSection = (id) => {
    ...
    setMobileMenuOpen(false); // Close mobile drawer menu after click
  };
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 premium-glass border-b border-clinic-accent/10 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-2 md:gap-3 shrink-0">
            <img src="/logo.jpg" alt="Radev Clinic Logo" className="w-8 h-8 md:w-10 h-10 object-contain rounded-lg shadow-sm" />
            <div>
              <span className="font-display text-base md:text-2xl font-bold tracking-wide text-clinic-navy">РАДЕВ</span>
              <span className="font-display text-base md:text-2xl font-light text-clinic-accent ml-1">Клиник</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <button 
              ...
              className="flex md:hidden p-2 text-clinic-navy hover:bg-clinic-accent/10 rounded-xl transition-colors duration-200"
            >
        ...
        {mobileMenuOpen && (
          <div className="md:hidden premium-glass border-b border-clinic-accent/10 py-6 px-6 flex flex-col space-y-4 animate-fadeIn">
            <button onClick={() => { window.scrollTo({ top: 0, behavior: 'smooth' }); setMobileMenuOpen(false); }} className="text-left py-2 border-b border-clinic-accent/5 font-semibold text-clinic-navy hover:text-clinic-accent text-sm outline-none">Начало</button>
            <button onClick={() => scrollToSection('services')} className="text-left py-2 border-b border-clinic-accent/5 font-semibold text-clinic-navy hover:text-clinic-accent text-sm outline-none">Услуги</button>
            <button onClick={() => scrollToSection('about')} className="text-left py-2 border-b border-clinic-accent/5 font-semibold text-clinic-navy hover:text-clinic-accent text-sm outline-none">За нас</button>
            <button onClick={() => scrollToSection('stats')} className="text-left py-2 border-b border-clinic-accent/5 font-semibold text-clinic-navy hover:text-clinic-accent text-sm outline-none">Доверие</button>
            <button onClick={() => scrollToSection('contact')} className="text-left py-2 font-semibold text-clinic-navy hover:text-clinic-accent text-sm outline-none">Контакти</button>
          </div>
        )}
      </nav>
